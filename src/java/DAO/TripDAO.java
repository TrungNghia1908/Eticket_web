...
//            ps.setString(2, tripName);
    ...
    public static List<TripList> selectTripTable() {
        ConnectionPool pool = ConnectionPool.getInstance();
        Connection connection = pool.getConnection();
        PreparedStatement ps = null;
        ResultSet rs = null;
        
        String query = "SELECT DISTINCT Arrival FROM trip ";
        ...
        try {
            ...
            // Because the result (rs) is the list of trip 
            // so creating a list to store result.
            ArrayList<TripList> table = new ArrayList<>();
            while (rs.next()) {
                // Create trip to store each row of the result.
                String tripListName = rs.getString("Arrival");
                TripList tl = select(tripListName);
                // add the trip to the trip List.
                table.add(tl);
            }
            return table;
